...
import { Bell, Box, ChevronRight, CircleUserRound, Home as HomeIcon, Plus, Settings2, Snowflake, UserRound } from 'lucide-react';
import { api, clearSession, connectTelemetry, getUser, setSession } from './api';
import { Badge, Button, Card as UiCard, CardHeader, Input, Progress, Select } from './ui';
...
function Home({ state }) {
  ...
  return <Shell page="home" state={state}><div className="greeting"><div><h1>Namaste, {state.user?.name || 'Farmer'} 🙏</h1><p>{unit.village}, {unit.district}, {unit.state}</p></div><Link href="/scan" className="text-link">Switch unit →</Link></div><Card className="capacity"><div className="section-head"><b>Current storage</b><span>{pct.toFixed(0)}% occupied</span></div><div className="capacity-row"><strong>{unit.occupied_capacity.toFixed(1)} kg</strong><span>{unit.available_capacity.toFixed(1)} kg available<br />of {unit.total_capacity.toFixed(0)} kg</span></div><div className="progress"><i style={{width:`${Math.min(pct,100)}%`}} /></div><div className="meta-row"><span>Capacity {unit.total_capacity} kg</span><span>Shared chamber</span></div></Card><div><div className="eyebrow">Current condition</div><div className="metric-grid"><Metric label="Inside temp" value={`${(unit.current_temp ?? 11.6).toFixed(1)}°C`} sub="Target 11.5°C" /><Metric label="Humidity" value={`${(unit.current_humidity ?? 78).toFixed(0)}%`} sub="Safe 75–90%" /><Metric label="Battery" value={`${(unit.battery_percentage ?? 82).toFixed(0)}%`} sub="LiFePO4 Solar" /><Metric label="Door" value={unit.door_open ? 'Open' : 'Closed'} sub="Magnetic seal" /></div></div><div className="action-grid"><Link href="/scan">⌕<span>Scan storage</span></Link><Link href="/add-produce" className="primary-action">＋<span>Add produce</span></Link><Link href="/live-storage">⌁<span>Live status</span></Link></div><Card><div className="section-head"><b>Active alerts</b><Link href="/alerts">View all →</Link></div>{alerts[0] ? <AlertRow alert={alerts[0]} /> : <Empty title="Storage operating normal" text="Solar cooling and humidity are within safe bands." />}</Card><Card><div className="section-head"><b>Current produce</b><span>Chamber inventory</span></div>{records.length ? records.map(r => <div className="list-row" key={r.id}><span className="crop-icon">{icons[r.crop_name] || '📦'}</span><span><b>{r.crop_name}</b><small>Stored {r.storage_age_human} · {r.farmer_name}</small></span><strong>{r.quantity_kg} kg</strong></div>) : <Empty title="No produce stored" text="Add a harvest batch to begin." />}</Card><Card><div className="section-head"><b>Decision support</b><Link href="/recommendation">Full analysis →</Link></div><div className="recommendation"><b>{recs[0]?.decision || 'STORE'}</b><p>{recs[0]?.primary_reason || 'Storage conditions are optimal. Produce is safely preserved.'}</p></div></Card></Shell>;
}
...
function Simulation({ state, compact=false }) { const [open,setOpen]=useState(false);async function control(payload){await api.controlSimulation(payload);state.notify('Simulation updated','success');setOpen(false)}return <>{<button className={compact?'secondary compact':'secondary'} onClick={()=>setOpen(true)}>Demo controls</button>}{open&&<div className="modal-backdrop"><Card className="modal"><div className="section-head"><b>Hardware demo controls</b><button className="icon-button" onClick={()=>setOpen(false)}>×</button></div><p className="muted">Trigger sensor conditions without physical hardware.</p><div className="button-grid"><button onClick={()=>control({force_temperature:16.5})}>🔥 Force 16.5°C</button><button onClick={()=>control({force_temperature:11.5})}>❄ Reset 11.5°C</button><button onClick={()=>control({force_door_open:true})}>🚪 Open door</button><button onClick={()=>control({force_door_open:false})}>🔒 Seal door</button><button onClick={()=>control({force_battery_pct:22})}>🪫 Drain to 22%</button><button onClick={()=>control({force_battery_pct:85})}>🔋 Restore battery</button></div></Card></div>}</> }
...
function History({ state }) { const [items,setItems]=useState([]);const [all,setAll]=useState(false);function load(){api.history(all).then(setItems)}useEffect(load,[all]);return <Shell page="history" state={state}><PageTitle title="Storage history & aging"/><div className="tabs"><button className={!all?'selected':''} onClick={()=>setAll(false)}>My produce</button><button className={all?'selected':''} onClick={()=>setAll(true)}>Entire chamber</button></div>{items.length?items.map(r=><Card key={r.id}><div className="section-head"><b>{r.crop_name} · {r.quantity_kg} kg</b><em>{r.status}</em></div><p>{r.unit_code} · stored {r.storage_age_human} ago</p><p className="muted">Safe shelf-life left: {r.remaining_safe_human}</p>{r.status==='stored'&&<button className="secondary" onClick={()=>api.checkout(r.id).then(load)}>Retrieve / checkout</button>}</Card>):<Card><Empty title="No storage history" text="No produce batches found."/></Card>}</Shell>; }

function Market({ state }) { const [market,setMarket]=useState([]);const [transport,setTransport]=useState([]);useEffect(()=>{Promise.all([api.market(),api.transport()]).then(([m,t])=>{setMarket(m);setTransport(t)})},[]);return <Shell page="market" state={state}><PageTitle title="Market & logistics"/><div className="eyebrow">Regional mandi rates</div>{market.map(m=><Card key={m.id}><div className="section-head"><b>{m.crop_name}</b><em>{m.price_trend}</em></div><p>{m.market_name}, {m.district} · {m.distance_km} km</p><h2>₹{m.modal_price.toFixed(0)} <small>/ quintal</small></h2></Card>)}<div className="eyebrow">Rural freight</div>{transport.map(t=><Card key={t.id}><b>🚚 {t.transport_mode}</b><p>{t.origin_village} → {t.destination_market}</p><p className="muted">{t.estimated_hours} hours · ₹{t.cost_per_quintal}/qtl · {t.departure_time}</p><a className="secondary button-link" href={`tel:${t.provider_phone}`}>Call driver</a></Card>)}</Shell>; }
